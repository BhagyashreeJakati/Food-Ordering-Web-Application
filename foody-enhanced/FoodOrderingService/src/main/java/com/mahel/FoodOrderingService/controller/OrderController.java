...
public class OrderController {
    ...
    @GetMapping("/user")
    public ResponseEntity<ResponseDTO<List<Order>>> getOrderHistory(
            @RequestHeader("Authorization") String jwt) throws Exception {
    ...
    public ResponseEntity<ResponseDTO<String>> cancelOrder(
            @PathVariable Long orderId,
            @RequestHeader("Authorization") String jwt) throws Exception {
        ResponseDTO<String> response = new ResponseDTO<>();
        userService.userByToken(jwt);
        orderService.updateOrder(orderId, "CANCELLED");
        response.setPayload("Order #" + orderId + " cancelled");
