...
> 🔒 All credentials (DB password, JWT secret, Razorpay keys, mail credentials) are loaded from environment variables — nothing sensitive is hardcoded or committed to the repo.
---
...
Credentials are read from environment variables, not committed to the repo. Copy the example file and fill in your own values:
```bash
...
Edit `.env`:
```bash
DB_URL=jdbc:mysql://localhost:3306/food_ordering_system?createDatabaseIfNotExist=true
DB_USERNAME=root
DB_PASSWORD=YOUR_MYSQL_PASSWORD

JWT_SECRET_KEY=replace-with-a-long-random-secret-key-at-least-64-characters-long
...
Load the `.env` file before running the backend (e.g. `export $(cat .env | xargs)` on macOS/Linux, or use an IDE plugin like EnvFile in IntelliJ).
...
🔑 Demo Accounts
Role	Email	Password
Restaurant Owner (Admin)	admin@foody.com	ChangeMe@123! (or your `ADMIN_SEED_PASSWORD` value)
...
⚠️ Troubleshooting
Problem	Fix
Backend won't start	Check MySQL is running on the port set in `DB_URL`; verify `.env` is loaded as environment variables
...
Wrong MySQL port	Update `DB_URL` in your `.env` file to match your local MySQL port
---
